...
这次，我们将讨论编程初学者的“鬼门”，指针。我认为比起 C 语言，遇到困难的人并不多，但仍然会有不少人感到困难，因此我会仔细进行解说。

##  指针


**指针/pointer** 是一个表示“指示某物”的词，表示变量等存在的“位置”。如下所示使用。
...
计算机的核心是进行计算的 **CPU（英特尔的那种）**和用于存储数据的内存（8GB 或 16GB 等）。它们通常被比作人类的大脑和桌面上的工作区域。大脑越聪明，计算越快；桌面越大，工作空间越宽裕。
...
指针类型的零值是 **nil（尼尔）**这个特殊值。nil 表示不指向任何地方的无效值。
...
指针只是一个数字，因此只需复制指针，就可以相对快速地在函数中通过指针使用该巨大数据。

![大きなデータとポインター](18_pointer.assets/大きなデータとポインター.png)
