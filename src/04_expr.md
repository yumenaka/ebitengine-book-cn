...
**禁止重复声明变量**，严格意义上来说是“在同一作用域内”不能再次声明同一变量。

但在不同作用域中，可以声明同名变量。重名的时候，更小的作用域中声明的同名变量，会优先生效。因此范围更广的变量会被隐藏，在该作用域内无法访问。这被称为**变量遮蔽**。

编程中，**变量遮蔽**（英语：variable shadowing，或称变量隐藏）指的是：当新变量与旧有变量同名，此名称暂时不再可用于访问旧有变量。

不过，在大多数情况下，变量的声明和赋值会同时进行。声明和赋值同时进行时，可以省略变量类型，写起来更省事。

无论用哪一种语法，变量总是可以多次赋值，这一点不会变。
...
此外，在函数内部，可以使用符号 `:=` 来省略 `var` 这个关键字，使得声明和赋值的语句更简洁，这是最常用的语法。
 ...
 * `hp = hp + 20` 这个等式，在数学中根本就不成立。但编程领域的 `=` 号，其实是表示代入（覆盖）的符号。请把这里的等号，当作和数学里的等号不同的符号来看。如果实在无法适应，请在心中将 `=` 替换为 `⇐`（表示数据流动方向的箭头） 吧。

##  4.8 函数

函数是某些处理的集合。例如 `fmt.Println` 函数，是执行在屏幕上显示文本的处理的函数。

函数可以接收 `()` 中的输入。这个输入称为参数。给函数提供输入，称为传递参数（简称“传参”）。

同时传递多个参数，这是需要用逗号 `,` 分隔。例如 `fmt.Println` 函数，就可以将多个参数用空格连接，并把这些内容显示到屏幕上。
